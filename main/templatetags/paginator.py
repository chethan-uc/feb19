...
def get_paginator(paginator, page_obj):
	...
	if page_obj.paginator.num_pages <= LEADING_PAGE_RANGE_DISPLAYED:
		...
		page_numbers = [n for n in range(1, page_obj.paginator.num_pages + 1) if n > 0 and n <= page_obj.paginator.num_pages]
	elif page_obj.number <= LEADING_PAGE_RANGE:
		in_leading_range = True
		page_numbers = [n for n in range(1, LEADING_PAGE_RANGE_DISPLAYED + 1) if n > 0 and n <= page_obj.paginator.num_pages]
		...
		page_numbers = [n for n in range(page_obj.paginator.num_pages - TRAILING_PAGE_RANGE_DISPLAYED + 1, page_obj.paginator.num_pages + 1) if n > 0 and n <= page_obj.paginator.num_pages]
